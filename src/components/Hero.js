...
const Hero = () => {
  ...
  // Pointer-driven parallax for the ID badge (normalized -0.5..0.5)
  const px = useMotionValue(0);
  const py = useMotionValue(0);
  const springCfg = { stiffness: 150, damping: 18, mass: 0.4 };
  const rotateY = useSpring(useTransform(px, [-0.5, 0.5], [-11, 11]), springCfg);
  const rotateX = useSpring(useTransform(py, [-0.5, 0.5], [9, -9]), springCfg);
  const layerX = useSpring(useTransform(px, [-0.5, 0.5], [-16, 16]), springCfg);
  const layerY = useSpring(useTransform(py, [-0.5, 0.5], [-12, 12]), springCfg);
  const glareX = useTransform(px, [-0.5, 0.5], ['0%', '100%']);
  const glareBg = useTransform(
    glareX,
    (x) => `radial-gradient(420px circle at ${x} 0%, rgba(255,255,255,0.28), transparent 60%)`
  );

  const handleBadgeMove = (e) => {
        ...
        <div className="grid md:grid-cols-2 gap-10 items-center">
          ...
          >
            ...
            <h1 className="mt-6 text-4xl md:text-6xl font-bold text-secondary-900 leading-tight text-shadow-soft">
              Wai Yan{' '}
              <DecryptText text="Bo" className="gradient-text" speed={55} />
            </h1>
          ...
          {/* Right Column — Interactive ID Badge with pointer parallax */}
          <div className="perspective-1000" onMouseMove={handleBadgeMove} onMouseLeave={resetBadge}>
            <motion.div
            ...
            >
              ...
              {/* Pointer glare */}
              <motion.div
                className="pointer-events-none absolute inset-0 rounded-2xl z-20 opacity-0 group-hover:opacity-100 transition-opacity duration-300"
              ...
              <div className="relative bg-white rounded-2xl border border-secondary-200 shadow-lg overflow-hidden flex flex-col">
                ...
                {/* Photo — lifted on its own parallax plane */}
                <motion.div
                  className="px-6 -mt-12 relative z-10 flex justify-between items-end"
